...
let process_file filename =
  ...
  let rec read_lines current_section seeds seed_to_soil_map =
    try
      let line = input_line in_channel in
      if String.trim line = "" then
        read_lines current_section seeds seed_to_soil_map
      else if String.length line >= 6 && String.sub line 0 6 = "seeds:" then
        read_lines "seeds" (List.append seeds (parse_line_to_list (String.sub line 7 (String.length line - 7)))) seed_to_soil_map
...
(* Main function *)
let () =
  let (seeds, seed_to_soil_map) = process_file "example.txt" in
  (* Print the seeds and maps *)
  Printf.printf "Seeds: ";
  List.iter (Printf.printf "%d ") seeds;
  Printf.printf "\nSeed to Soil Map:\n";
  List.iter (fun (a, b, c) -> Printf.printf "%d %d %d\n" a b c) seed_to_soil_map;
